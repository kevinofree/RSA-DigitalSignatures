...
def getFileSig(fileName, privKey):
	...
	fileHash = SHA512.new(fileContents).hexdigest()
	sig = digSig(privKey, fileHash)
	return sig
...
def verifyFileSig(fileName, pubKey, signature):
	...
	fileContents = open(fileName, 'r').read()
	fileHash = SHA512.new(fileContents).hexdigest()
	if verifySig(fileHash, signature, pubKey) == True:
		return True
	else:
		return False
...
def saveSig(fileName, signature):
	...
	# by fileName)
	open(fileName, 'w').write(str(signature[0]))
